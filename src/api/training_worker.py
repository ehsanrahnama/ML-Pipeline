...
def load_data(data_path=None):
    ...
    if data_path and os.path.exists(data_path):
        ...
        feature_cols = ['request_rate', 'active_users', 'db_connections', 
...
def objective(trial, X_train, X_test, y_train, y_test):
    ...
    param = {
        ...
        "n_estimators": trial.suggest_int("n_estimators", 50, 500, step=50),  # Reduced range as CPU data is simpler
        "learning_rate": trial.suggest_float("learning_rate", 1e-2, 0.3, log=True),  # Wider range
        "max_depth": trial.suggest_int("max_depth", 2, 8),  # Reduced depth as features are already engineered
        "subsample": trial.suggest_float("subsample", 0.7, 1.0),  # Increased min subsample
        "colsample_bytree": trial.suggest_float("colsample_bytree", 0.7, 1.0),  # Increased min colsample
        "min_child_weight": trial.suggest_int("min_child_weight", 1, 7),  # Reduced range
        "gamma": trial.suggest_float("gamma", 1e-8, 0.5, log=True),  # Reduced upper bound
        "reg_alpha": trial.suggest_float("reg_alpha", 1e-8, 0.5, log=True),  # L1 regularization
        "reg_lambda": trial.suggest_float("reg_lambda", 1e-8, 0.5, log=True),  # L2 regularization
    }
...
def train_model(job_id, payload, redis_host="localhost"):
    ...
    try:
        ...
        # Train final model
        best_model = xgb.XGBRegressor(**study.best_params, n_jobs=1, objective="reg:squarederror")
        ...
        # Save model & register
        version, model_path = save_model_and_register(best_model, study.best_params, metrics)
        ...
        logger.info(f"Training completed: {version} (RMSE={rmse:.3f}), elapsed time: {elapsed:.1f}s")

        
    except Exception as e:
        elapsed = time.time() - start_time
        r.hset(f"job:{job_id}", mapping={"status": "failed", "error": str(e) , "elapsed_time": f"{elapsed:.1f}"})
